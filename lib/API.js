...
DRONE.API = (function() {
  ...
  var commandCreator = new DRONE.CommandCreator();

  var parser = new PaVEParser();

  function streamVideo(connectionResult) {
    var vidSock = sockets['vid'].socket;
    var onReadHandler = function(readInfo) {
      if (readInfo.resultCode > 0) {
        parser.parse(readInfo.data);
      }
      chrome.socket.read(vidSock, null, onReadHandler);
    };
  ...
  function init(cbConnected, cbConnectionError) {
    ...
    connect(sockets['vid'], streamVideo);
   ...
   */
  function connect(sockRef, cb) {
    cb = cb || callbacks.onConnected;
  ...
  function sendSensitivity() {
    sendCommands([
      commandCreator.config('control:euler_angle_max', '0.1'),
      commandCreator.config('control:indoor_euler_angle_max', '0.1'),
      commandCreator.config('control:outdoor_euler_angle_max', '0.1')
    ]);
  }
  ...
  function sendOutdoor(outdoor) {
    sendCommands([commandCreator.config('control:outdoor', outdoor?'TRUE':'FALSE')]);
  }
  ...
  function flip(direction) {
    switch (direction) {
      case 'left':
        commands = [commandCreator.animate('flipLeft', 10)];
        break;
      case 'right':
        commands = [commandCreator.animate('flipRight', 10)];
        break;
      case 'front':
        commands = [commandCreator.animate('flipAhead', 10)];
        break;
      case 'back':
        commands = [commandCreator.animate('flipBehind', 10)];
        break;
    }
    // send and schedule the update
    sendCommands(commands);
  ...
  function led_animate(sequence) {
    switch (sequence) {
      case 1:
        commands = [commandCreator.animateLeds('frontLeftGreenOthersRed')];
        break;
      case 2:
        commands = [commandCreator.animateLeds('frontRightGreenOthersRed')];
        break;
      case 3:
        commands = [commandCreator.animateLeds('rearRightGreenOthersRed')];
        break;
      case 4:
        commands = [commandCreator.animateLeds('rearLeftGreenOthersRed')];
        break;
    }
    // send and schedule the update
    sendCommands(commands);

    setTimeout(led_animate, 60);
  }
