...
DRONE.API = (function() {
  ...
  function sendKeepAliveCommand() {
      ...
      chrome.socket.read(
        ...
        function(data) {
          ...
          if(data.data.byteLength > 0) {
            navdata = DRONE.NavData.parse(data.data);
            navlog(JSON.stringify(navdata));
          } else {
        //    shutdown();
          }
        });
    ...
    setTimeout(sendKeepAliveCommand, 200);
  }
  ...
  function animateNext() {
    sendCommands([commandCreator.animateNext()]);
    setTimeout(animateNext, 60);
  }

  function animatePrev() {
    sendCommands([commandCreator.animatePrev()]);
    setTimeout(animatePrev, 60);
  }
