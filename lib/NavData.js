...
DRONE.NavData = function () {
  ...
  this.OPTION_PARSERS = {
    'demo': function(reader) {
      ...
      var flyState          = this.FLY_STATES[reader.uint16()];
      var controlState      = this.CONTROL_STATES[reader.uint16()];
  ...
  this.NAVDATA_HEADER = 0x55667788;
...
DRONE.NavData.prototype.timesMap = function(n, iterator, context) {
  var results = [];
  for (var i = 0; i < n; i++) results[i] = iterator.call(context, i);
  return results;
};

DRONE.NavData.prototype.droneTimeToMilliSeconds = function(time) {
...
DRONE.NavData.prototype.parseNavdata = function(buffer) {
  reader = new DRONE.NavdataReader(buffer);

  var navdata = {};

  navdata.header = reader.uint32();
  if (navdata.header !== this.NAVDATA_HEADER) {
    throw new Error('Invalid header: 0x' + navdata.header.toString(16));
  }

  navdata.droneState     = this.reader.mask32(this.DRONE_STATES);
  navdata.sequenceNumber = this.reader.uint32();
  navdata.visionFlag     = this.reader.uint32();

  while (true) {
    var optionId   = this.reader.uint16();
    var optionName = this.OPTION_IDS[optionId];
    var length     = this.reader.uint16();

    // length includes header size (4 bytes)
    var optionReader = new DRONE.NavdataReader(reader.buffer(length - 4));
    ...
    navdata[optionName] = (this.OPTION_PARSERS[optionName])
      ? this.OPTION_PARSERS[optionName](optionReader)
